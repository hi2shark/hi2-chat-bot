...
**2.3.3**版，增强稳定性与监控能力  
  ...
  - `polling_error` 发生时自动执行轮询重启，减少长时间“假死”  
  - 与 Uptime Kuma 联动：仅在 TG 轮询健康时才 push 状态，避免出现“监控正常但 Bot 已不工作”的情况

**2.3.2**版，新增智能审核模式  
  - 当同时开启人机验证和AI审核的时候，先进行AI判定内容性质，再决定处理方案
    - 广告内容：直接拉黑
    - 无意义内容：触发人机验证，验证通过后转发消息
    - 正常内容：直接转发消息
...
- Telegram 客户端现在支持循环定时消息，建议给本机器人设置循环发送 `/status` 或 `/ping` 等指令，用来持续验证机器人与 Telegram 直连链路是否正常（相比仅看 Uptime Kuma，更贴近真实使用状态）
...
当AI审核和人机验证同时启用时，自动开启智能审核模式：
- **广告内容**：AI判定为广告的用户将被直接拉黑
- **无意义内容**：AI判定为无意义内容（如随机字符、测试消息等）的消息将触发人机验证
  - 此类消息不计入审核通过次数
  - 通过人机验证后转发保存的消息
  - 下次发送内容将继续触发AI审核
- **正常内容**：计入审核通过次数，达到设定次数后通过审核

智能模式让机器人更聪明地区分恶意用户、测试行为和正常用户，提供更精准的内容管理。  
...
| `AI_AUDIT_ENABLED` | 是否启用AI审核功能，填入1启用。与`CAPTCHA_ENABLED`同时启用时自动开启智能审核模式 | `0`（不启用） |
